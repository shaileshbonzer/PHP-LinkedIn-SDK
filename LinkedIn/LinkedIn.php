...
class LinkedIn
{
    ...
    const SCOPE_READ_ORGANIZATION_SOCIAL = 'r_organization_social';
    const SCOPE_WRITE_ORGANIZATION_SOCIAL = 'W_organization_social';
    ...
    protected function _makeRequest($url, array $payload = array(), $method = 'GET', array $headers = array(), array $curl_options = array())
    {
        ...
        if (!empty($curl_options)) {
            $options = array_merge_recursive($options, $curl_options);
        }

        if (isset($this->_config['curl_options']) && !empty($this->_config['curl_options'])) {
            $options = array_merge_recursive($options, $this->_config['curl_options']);
        }
